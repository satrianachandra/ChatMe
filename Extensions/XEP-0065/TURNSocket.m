...
**/
- (id)initWithStream:(XMPPStream *)stream toJID:(XMPPJID *)aJid withID:(NSString *)theID
{
	if ((self = [super init]))
	{
		XMPPLogTrace();
...
- (id)initWithStream:(XMPPStream *)stream incomingTURNRequest:(XMPPIQ *)iq
{
	if ((self = [super init]))
	{
        ...
        //..modified to sid, by me
        NSXMLElement *query2 = [iq elementForName:@"query" xmlns:@"http://jabber.org/protocol/bytestreams"];
        sidChan = [query2 attributeStringValueForName:@"sid"];
...
- (void)sendReply
{
	...
	NSXMLElement *query = [NSXMLElement elementWithName:@"query" xmlns:@"http://jabber.org/protocol/bytestreams"];
	//[query addAttributeWithName:@"sid" stringValue:uuid];
	//[query addAttributeWithName:@"sid" stringValue:sidChan];
...
- (void)targetNextConnect
{
	...
	if(streamhostIndex < [streamhosts count])
	{
        ...
        //proxyJID = [XMPPJID jidWithString:@"proxy.eueung-mulyanas-imac.local"];
		//NSLog(@"proxyJID: %@",proxyJID);
		
        proxyHost = [[streamhost attributeForName:@"host"] stringValue];
        //proxyHost = @"127.0.0.1";
       // NSLog(@"proxyHost: %@",proxyHost);
...
- (void)socksConnect
{
    ...
    //NSString *hashMe = [NSString stringWithFormat:@"%@%@%@", sidChan, [initiatorJID full], [targetJID full]];
	NSString *hashMe;
    if (isClient){
        hashMe = [NSString stringWithFormat:@"%@%@%@", uuid, [initiatorJID full], [targetJID full]];
    }else{
        hashMe = [NSString stringWithFormat:@"%@%@%@", sidChan, [initiatorJID full], [targetJID full]];
    }
